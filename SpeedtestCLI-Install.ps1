...
# Ce script installe Ookla Speedtest CLI en utilisant le gestionnaire de paquets Chocolatey.
...
Write-Host "=================================================="
Write-Host " Début du script d'installation d'Ookla Speedtest CLI (Chocolatey)"
Write-Host "=================================================="
Write-Host ""
...
# --- Vérifier si Ookla Speedtest CLI est déjà installé ---
Write-Host "--- Vérification de l'installation existante d'Ookla Speedtest CLI ---"
...
try {
    ...
    if ($installedPackages -like "*$packageName*") {
        ...
        try {
            Get-Command speedtest.exe -ErrorAction Stop | Out-Null
            Write-Host "La commande 'speedtest' est reconnue."
        } catch {
             Write-Warning "Le paquet $packageName est installé, mais la commande 'speedtest' n'est pas immédiatement reconnue dans cette session."
        ...
        Write-Host ""
        Write-Host "=================================================="
        Write-Host " Script d'installation d'Ookla Speedtest CLI terminé (déjà installé)!"
        Write-Host "=================================================="
        Exit 0
    } else {
...
# --- Installation d'Ookla Speedtest CLI via Chocolatey ---
...
try {
    ...
    Write-Host "Vérification de la reconnaissance de la commande 'speedtest'..."

    Start-Sleep -Seconds 5

     try {
        Get-Command speedtest.exe -ErrorAction Stop | Out-Null
        Write-Host "La commande 'speedtest' est maintenant reconnue dans cette session (ou le sera après un redémarrage de session)."
    } catch {
         Write-Warning "Le paquet $packageName a été installé, mais la commande 'speedtest' n'est pas immédiatement reconnue dans cette session."
         Write-Warning "Ceci est normal. Vous devrez ouvrir une nouvelle session PowerShell ou une nouvelle fenêtre Terminal"
         Write-Warning "pour que la commande 'speedtest' soit ajoutée à votre PATH et reconnue."
         Write-Host "Vérifiez l'installation en ouvrant un nouveau Terminal et en tapant 'speedtest --version'."
    }
...
Write-Host "=================================================="
Write-Host " Script d'installation d'Ookla Speedtest CLI terminé !"
Write-Host "=================================================="
Write-Host "$packageName a été installé avec succès."
Write-Host "Ouvrez une nouvelle session PowerShell/Terminal pour utiliser 'speedtest'."
Write-Host ""